...
void commonConstruct(Proxy &node, ProxyType type, const std::string &group,
                     const std::string &remarks, const std::string &server,
                     const std::string &port, const tribool &udp,
                     const tribool &tfo, const tribool &scv,
                     const tribool &tls13,
...
void vmessConstruct(
    Proxy &node, const std::string &group, const std::string &remarks,
    const std::string &add, const std::string &port, const std::string &type,
    const std::string &id, const std::string &aid, const std::string &net,
    const std::string &cipher, const std::string &path, const std::string &host,
  ...
  commonConstruct(node, ProxyType::VMess, group, remarks, add, port, udp, tfo,
                  scv, tls13, underlying_proxy);
  ...
  if (net == "quic") {
    ...
    node.Host = (host.empty() && !isIPv4(add) && !isIPv6(add)) ? add.data()
                                                               : trim(host);
...
void ssrConstruct(Proxy &node, const std::string &group,
                  const std::string &remarks, const std::string &server,
                  const std::string &port, const std::string &protocol,
                  const std::string &method, const std::string &obfs,
                  const std::string &password, const std::string &obfsparam,
                  const std::string &protoparam, tribool udp, tribool tfo,
                  tribool scv, const std::string &underlying_proxy) {
  commonConstruct(node, ProxyType::ShadowsocksR, group, remarks, server, port,
...
void ssConstruct(Proxy &node, const std::string &group,
                 const std::string &remarks, const std::string &server,
                 const std::string &port, const std::string &password,
                 const std::string &method, const std::string &plugin,
                 const std::string &pluginopts, tribool udp, tribool tfo,
                 tribool scv, tribool tls13,
                 const std::string &underlying_proxy) {
  commonConstruct(node, ProxyType::Shadowsocks, group, remarks, server, port,
...
void socksConstruct(Proxy &node, const std::string &group,
                    const std::string &remarks, const std::string &server,
                    const std::string &port, const std::string &username,
                    const std::string &password, tribool udp, tribool tfo,
                    tribool scv, const std::string &underlying_proxy) {
  commonConstruct(node, ProxyType::SOCKS5, group, remarks, server, port, udp,
                  tfo, scv, tribool(), underlying_proxy);
  node.Username = username;
  node.Password = password;
}

void httpConstruct(Proxy &node, const std::string &group,
                   const std::string &remarks, const std::string &server,
                   const std::string &port, const std::string &username,
                   const std::string &password, bool tls, tribool tfo,
                   tribool scv, tribool tls13,
                   const std::string &underlying_proxy) {
  commonConstruct(node, tls ? ProxyType::HTTPS : ProxyType::HTTP, group,
                  remarks, server, port, tribool(), tfo, scv, tls13,
                  underlying_proxy);
  node.Username = username;
  node.Password = password;
  node.TLSSecure = tls;
...
void trojanConstruct(Proxy &node, const std::string &group,
                     const std::string &remarks, const std::string &server,
                     const std::string &port, const std::string &password,
                     const std::string &network, const std::string &host,
                     const std::string &path, const std::string &fp,
                     const std::string &sni,
                     const std::vector<std::string> &alpnList, bool tlssecure,
                     tribool udp, tribool tfo, tribool scv, tribool tls13,
                     const std::string &underlying_proxy) {
  commonConstruct(node, ProxyType::Trojan, group, remarks, server, port, udp,
                  tfo, scv, tls13, underlying_proxy);
  node.Password = password;
  node.Host = host;
...
void snellConstruct(Proxy &node, const std::string &group,
                    const std::string &remarks, const std::string &server,
                    const std::string &port, const std::string &password,
                    const std::string &obfs, const std::string &host,
                    uint16_t version, tribool udp, tribool tfo, tribool scv,
                    const std::string &underlying_proxy) {
  commonConstruct(node, ProxyType::Snell, group, remarks, server, port, udp,
                  tfo, scv, tribool(), underlying_proxy);
  node.Password = password;
  node.OBFS = obfs;
  node.Host = host;
...
void wireguardConstruct(Proxy &node, const std::string &group,
                        const std::string &remarks, const std::string &server,
                        const std::string &port, const std::string &selfIp,
                        const std::string &selfIpv6, const std::string &privKey,
                        const std::string &pubKey, const std::string &psk,
                        const string_array &dns, const std::string &mtu,
                        const std::string &keepalive,
                        const std::string &testUrl, const std::string &clientId,
                        const tribool &udp,
                        const std::string &underlying_proxy) {
  commonConstruct(node, ProxyType::WireGuard, group, remarks, server, port, udp,
                  tribool(), tribool(), tribool(), underlying_proxy);
  node.SelfIP = selfIp;
...
void hysteriaConstruct(Proxy &node, const std::string &group,
                       const std::string &remarks, const std::string &add,
                       const std::string &port, const std::string &type,
                       const std::string &auth, const std::string &auth_str,
                       const std::string &host, const std::string &up,
                       const std::string &down, const std::string &alpn,
                       const std::string &obfsParam,
                       const std::string &insecure, const std::string &ports,
                       const std::string &sni, tribool udp, tribool tfo,
                       tribool scv, tribool tls13,
                       const std::string &underlying_proxy) {
  commonConstruct(node, ProxyType::Hysteria, group, remarks, add, port, udp,
                  tfo, scv, tls13, underlying_proxy);
  node.Auth = auth;
  node.Host =
      (host.empty() && !isIPv4(add) && !isIPv6(add)) ? add.data() : trim(host);
  node.UpMbps = up;
  node.DownMbps = down;
  node.Alpn = alpn;
  node.OBFSParam = obfsParam;
...
void anyTlSConstruct(Proxy &node, const std::string &group,
                     const std::string &remarks, const std::string &port,
                     const std::string &password, const std::string &host,
                     const std::vector<String> &AlpnList,
                     const std::string &fingerprint, const std::string &sni,
                     tribool udp, tribool tfo, tribool scv, tribool tls13,
                     const std::string &underlying_proxy,
                     uint16_t idleSessionCheckInterval,
                     uint16_t idleSessionTimeout, uint16_t minIdleSession) {
  commonConstruct(node, ProxyType::AnyTLS, group, remarks, host, port, udp, tfo,
                  scv, tls13, underlying_proxy);
  node.Host = trim(host);
  node.Password = password;
  node.AlpnList = AlpnList;
...
void mieruConstruct(Proxy &node, const std::string &group,
                    const std::string &remarks, const std::string &port,
                    const std::string &password, const std::string &host,
                    const std::string &ports, const std::string &username,
                    const std::string &multiplexing,
                    const std::string &transfer_protocol, tribool udp,
                    tribool tfo, tribool scv, tribool tls13,
                    const std::string &underlying_proxy) {
  commonConstruct(node, ProxyType::Mieru, group, remarks, host, port, udp, tfo,
                  scv, tls13, underlying_proxy);
  node.Host = trim(host);
  node.Password = password;
  node.Ports = ports;
  node.TransferProtocol =
      transfer_protocol.empty() ? "TCP" : trim(transfer_protocol);
  node.Username = username;
  node.Multiplexing =
      multiplexing.empty() ? "MULTIPLEXING_LOW" : trim(multiplexing);
}
...
void vlessConstruct(
    ...
    const std::string &add, const std::string &port, const std::string &type,
    const std::string &id, const std::string &aid, const std::string &net,
    const std::string &cipher, const std::string &flow, const std::string &mode,
    const std::string &path, const std::string &host, const std::string &edge,
    const std::string &tls, const std::string &pbk, const std::string &sid,
    const std::string &fp, const std::string &sni,
    const std::vector<std::string> &alpnList,
    const std::string &packet_encoding, tribool udp, tribool tfo, tribool scv,
    tribool tls13, const std::string &underlying_proxy,
    tribool v2ray_http_upgrade) {
  commonConstruct(node, ProxyType::VLESS, group, remarks, add, port, udp, tfo,
                  scv, tls13, underlying_proxy);
  node.UserId = id.empty() ? "00000000-0000-0000-0000-000000000000" : id;
  node.AlterId = to_int(aid);
  node.EncryptMethod = cipher;
  ...
  case "grpc"_hash:
    ...
    node.GRPCServiceName =
        path.empty() ? "/" : urlEncode(urlDecode(trim(path)));
    break;
  case "quic"_hash:
    node.Host = host;
...
void hysteria2Construct(Proxy &node, const std::string &group,
                        const std::string &remarks, const std::string &add,
                        const std::string &port, const std::string &password,
                        const std::string &host, const std::string &up,
                        const std::string &down, const std::string &alpn,
                        const std::string &obfsParam,
                        const std::string &obfsPassword, const std::string &sni,
                        const std::string &publicKey, const std::string &ports,
                        tribool udp, tribool tfo, tribool scv,
                        const std::string &underlying_proxy) {
  commonConstruct(node, ProxyType::Hysteria2, group, remarks, add, port, udp,
                  tfo, scv, tribool(), underlying_proxy);
  node.Password = password;
  node.Host =
      (host.empty() && !isIPv4(add) && !isIPv6(add)) ? add.data() : trim(host);
...
void tuicConstruct(Proxy &node, const std::string &group,
                   const std::string &remarks, const std::string &add,
                   const std::string &port, const std::string &password,
                   const std::string &congestion_control,
                   const std::string &alpn, const std::string &sni,
                   const std::string &uuid, const std::string &udpRelayMode,
                   const std::string &token, tribool udp, tribool tfo,
                   tribool scv, tribool reduceRtt, tribool disableSni,
                   uint16_t request_timeout,
                   const std::string &underlying_proxy) {
  commonConstruct(node, ProxyType::TUIC, group, remarks, add, port, udp, tfo,
...
void explodeVmess(std::string vmess, Proxy &node) {
  ...
  if (regMatch(vmess,
               "vmess://([A-Za-z0-9-_]+)\\?(.*)")) // shadowrocket style link
  {
  ...
  vmessConstruct(node, V2RAY_DEFAULT_GROUP, ps, add, port, type, id, aid, net,
                 "auto", path, host, "", tls, sni, std::vector<std::string>{});
}
  ...
  try {
    if (json.HasMember("outbounds")) // single config
    {
      if (json["outbounds"].Size() > 0 &&
        ...
        vmessConstruct(node, V2RAY_DEFAULT_GROUP, add + ":" + port, add, port,
                       type, id, aid, net, cipher, path, host, edge, tls, "",
                       std::vector<std::string>{}, udp, tfo, scv);
    ...
    // LOG_LEVEL_WARNING); return; ignore
    throw;
  }
  ...
  for (uint32_t i = 0; i < json["vmess"].Size(); i++) {
    ...
    case 1: // vmess config
      ...
      vmessConstruct(node, V2RAY_DEFAULT_GROUP, ps, add, port, type, id, aid,
                     net, cipher, path, host, "", tls, sni,
                     std::vector<std::string>{}, udp, tfo, scv);
      break;
    case 3: // ss config
      json["vmess"][i]["id"] >> id;
      json["vmess"][i]["security"] >> cipher;
      ssConstruct(node, SS_DEFAULT_GROUP, ps, add, port, id, cipher, "", "",
                  udp, tfo, scv);
      break;
    case 4: // socks config
      socksConstruct(node, SOCKS_DEFAULT_GROUP, ps, add, port, "", "", udp, tfo,
                     scv);
      break;
    default:
      continue;
...
void explodeSS(std::string ss, Proxy &node) {
  ...
  if (strFind(ss, "@")) {
    if (regGetMatch(ss, "(\\S+?)@(\\S+):(\\d+)", 4, 0, &secret, &server, &port))
      return;
    if (regGetMatch(urlSafeBase64Decode(secret), "(\\S+?):(\\S+)", 3, 0,
                    &method, &password))
      return;
  } else {
    if (regGetMatch(urlSafeBase64Decode(ss), "(\\S+?):(\\S+)@(\\S+):(\\d+)", 5,
                    0, &method, &password, &server, &port))
      return;
  }
  if (port == "0")
    return;
  ...
  ssConstruct(node, group, ps, server, port, password, method, plugin,
              pluginopts);
}
...
void explodeSSD(std::string link, std::vector<Proxy> &nodes) {
  ...
  for (uint32_t i = 0; i < listCount; i++) {
    ...
    Proxy node;
    ssConstruct(node, group, remarks, server, port, password, method, plugin,
                pluginopts);
    node.Id = index;
    nodes.emplace_back(std::move(node));
    index++;
...
void explodeSSConf(std::string content, std::vector<Proxy> &nodes) {
  ...
  if (json.HasParseError() || !json.IsObject())
    return;
  const char *section = json.HasMember("version") && json.HasMember("servers")
                            ? "servers"
                            : "configs";
...
void explodeSSR(std::string ssr, Proxy &node) {
  ...
  if (strFind(ssr, "/?")) {
    ...
    obfsparam = regReplace(urlSafeBase64Decode(getUrlArg(strobfs, "obfsparam")),
                           "\\s", "");
    protoparam = regReplace(
        urlSafeBase64Decode(getUrlArg(strobfs, "protoparam")), "\\s", "");
  }

  if (regGetMatch(ssr, "(\\S+):(\\d+?):(\\S+?):(\\S+?):(\\S+?):(\\S+)", 7, 0,
  ...
  if (find(ss_ciphers.begin(), ss_ciphers.end(), method) != ss_ciphers.end() &&
    ...
    ssrConstruct(node, group, remarks, server, port, protocol, method, obfs,
...
void explodeSSRConf(std::string content, std::vector<Proxy> &nodes) {
  Document json;
  std::string remarks, group, server, port, method, password, protocol,
      protoparam, obfs, obfsparam, plugin, pluginopts;
  auto index = nodes.size();
      ...
      json.HasMember("local_address")) // single libev config
  {
    ...
    if (find(ss_ciphers.begin(), ss_ciphers.end(), method) !=
      ...
      ssConstruct(node, SS_DEFAULT_GROUP, remarks, server, port, password,
                  method, plugin, pluginopts);
    } else {
      protoparam = GetMember(json, "protocol_param");
      obfsparam = GetMember(json, "obfs_param");
      ssrConstruct(node, SSR_DEFAULT_GROUP, remarks, server, port, protocol,
                   method, obfs, password, obfsparam, protoparam);
    }
    nodes.emplace_back(std::move(node));
    return;
  }
...
void explodeTrojan(std::string trojan, Proxy &node) {
  ...
  // format: trojan://{password}@{server}:{port}?type=ws&security=tls&path={path
  // (urlencoded)}&sni={host}#{name}
  ...
  trojanConstruct(node, group, remark, server, port, psk, network, host, path,
                  fp, sni, alpnList, true, tribool(), tfo, scv);
}
  ...
  if (configs[1] == "vmess") {
    ...
    for (uint32_t i = 6; i < configs.size(); i++) {
      ...
      case "obfs-header"_hash:
        headers =
            split(replaceAllDistinct(replaceAllDistinct(itemVal, "\"", ""),
                                     "[Rr][Nn]", "|"),
                  "|");
    ...
    vmessConstruct(node, group, ps, add, port, type, id, aid, net, cipher, path,
                   host, edge, tls, "", std::vector<std::string>{});
  }
}
...
void explodeNetch(std::string netch, Proxy &node) {
  ...
  std::string type, group, remark, address, port, username, password, method,
      plugin, pluginopts;
  std::string protocol, protoparam, obfs, obfsparam, id, aid, transprot,
      faketype, host, edge, path, tls, sni, fp;
  tribool udp, tfo, scv;
  ...
  case "SS"_hash:
    ...
    if (group.empty())
      group = SS_DEFAULT_GROUP;
    ssConstruct(node, group, remark, address, port, password, method, plugin,
                pluginopts, udp, tfo, scv);
    break;
  case "SSR"_hash:
    protocol = GetMember(json, "Protocol");
    obfs = GetMember(json, "OBFS");
    if (find(ss_ciphers.begin(), ss_ciphers.end(), method) !=
      ...
      if (group.empty())
        group = SSR_DEFAULT_GROUP;
      ssrConstruct(node, group, remark, address, port, protocol, method, obfs,
                   password, obfsparam, protoparam, udp, tfo, scv);
    }
    break;
  case "VMess"_hash:
    ...
    vmessConstruct(node, group, remark, address, port, faketype, id, aid,
                   transprot, method, path, host, edge, tls, sni,
                   std::vector<std::string>{}, udp, tfo, scv);
    break;
  case "Socks5"_hash:
    ...
    if (group.empty())
      group = SOCKS_DEFAULT_GROUP;
    socksConstruct(node, group, remark, address, port, username, password, udp,
                   tfo, scv);
    break;
  case "HTTP"_hash:
  case "HTTPS"_hash:
    if (group.empty())
      group = HTTP_DEFAULT_GROUP;
    httpConstruct(node, group, remark, address, port, username, password,
                  type == "HTTPS", tfo, scv);
    break;
  case "Trojan"_hash:
    host = GetMember(json, "Host");
    ...
    if (group.empty())
      group = TROJAN_DEFAULT_GROUP;
    trojanConstruct(node, group, remark, address, port, password, transprot,
                    host, path, fp, sni, std::vector<std::string>{},
                    tls == "true", udp, tfo, scv);
    break;
  case "Snell"_hash:
    obfs = GetMember(json, "OBFS");
    host = GetMember(json, "Host");
    ...
    snellConstruct(node, group, remark, address, port, password, obfs, host,
                   to_int(aid, 0), udp, tfo, scv);
    break;
  default:
    return;
  }
...
void explodeClash(Node yamlnode, std::vector<Proxy> &nodes) {
  ...
  for (uint32_t i = 0; i < yamlnode[section].size(); i++) {
    std::string proxytype, ps, server, port, cipher, group,
        password = "", ports, tempPassword; // common
    ...
    case "vmess"_hash:
      ...
      case "ws"_hash:
        if (singleproxy["ws-opts"].IsDefined()) {
          path = singleproxy["ws-opts"]["path"].IsDefined()
                     ? safe_as<std::string>(singleproxy["ws-opts"]["path"])
                     : "/";
      ...
      vmessConstruct(node, group, ps, server, port, "", id, aid, net, cipher,
                     path, host, edge, tls, sni, alpnList, udp, tfo, scv);
      break;
    case "ss"_hash:
      group = SS_DEFAULT_GROUP;
      ...
      if (cipher == "AEAD_CHACHA20_POLY1305")
        ...
        cipher = replaceAllDistinct(replaceAllDistinct(cipher, "AEAD_", ""),
                                    "_", "-");
        std::transform(cipher.begin(), cipher.end(), cipher.begin(), ::tolower);
      }

      ssConstruct(node, group, ps, server, port, password, cipher, plugin,
                  pluginopts, udp, tfo, scv);
      break;
    case "socks5"_hash:
      group = SOCKS_DEFAULT_GROUP;
    ...
    case "ssr"_hash:
      ...
      ssrConstruct(node, group, ps, server, port, protocol, cipher, obfs,
                   password, obfsparam, protoparam, udp, tfo, scv);
      break;
    case "http"_hash:
      group = HTTP_DEFAULT_GROUP;
      ...
      httpConstruct(node, group, ps, server, port, user, password,
                    tls == "true", tfo, scv);
      break;
    case "trojan"_hash:
      group = TROJAN_DEFAULT_GROUP;
      ...
      trojanConstruct(node, group, ps, server, port, password, net, host, path,
                      fp, sni, alpnList, true, udp, tfo, scv);
      break;
    case "snell"_hash:
      group = SNELL_DEFAULT_GROUP;
      ...
      snellConstruct(node, group, ps, server, port, password, obfs, host,
                     to_int(aid, 0), udp, tfo, scv);
      break;
    case "wireguard"_hash:
      ...
      wireguardConstruct(node, group, ps, server, port, ip, ipv6, private_key,
                         public_key, password, dns_server, mtu, "0", "", "",
                         udp, "");
      break;
    case "vless"_hash:
      ...
      case "ws"_hash:
        if (singleproxy["ws-opts"].IsDefined()) {
          ...
          if (singleproxy["ws-opts"]["v2ray-http-upgrade"].IsDefined()) {
            v2ray_http_upgrade = safe_as<std::string>(
      ...
      vlessConstruct(node, XRAY_DEFAULT_GROUP, ps, server, port, type, id, aid,
                     net, "auto", flow, mode, path, host, "", tls, pbk, sid, fp,
                     sni, alpnList, packet_encoding, udp, tribool(), tribool(),
                     tribool(), "", v2ray_http_upgrade);
      break;
    case "hysteria"_hash:
      ...
      sni = host;
      hysteriaConstruct(node, group, ps, server, port, type, auth, "", host, up,
                        down, alpn, obfsParam, insecure, ports, sni, udp, tfo,
                        scv);
      break;
    case "hysteria2"_hash:
      ...
      singleproxy["ports"] >> ports;
      sni = host;
      hysteria2Construct(node, group, ps, server, port, password, host, up,
                         down, alpn, obfsParam, obfsPassword, sni, public_key,
                         ports, udp, tfo, scv);
      break;
    case "tuic"_hash:
      ...
      tuicConstruct(node, TUIC_DEFAULT_GROUP, ps, server, port, password,
                    congestion_control, alpn, sni, id, udp_relay_mode, token,
                    tribool(), tribool(), scv, reduceRtt, disableSni,
                    request_timeout);
    ...
    case "anytls"_hash:
      ...
      if (!singleproxy["alpn"].IsNull() && singleproxy["alpn"].size() >= 1) {
        ...
        if (singleproxy["alpn"].size() >= 2 &&
            !singleproxy["alpn"][1].IsNull()) {
          singleproxy["alpn"][1] >>= alpn2;
          alpns.push_back(alpn2);
        }
      }
      singleproxy["client-fingerprint"] >>= fingerprint;
      anyTlSConstruct(node, ANYTLS_DEFAULT_GROUP, ps, port, password, server,
                      alpns, fingerprint, sni, udp, tribool(), scv, tribool(),
                      "", 30, 30, 0);
      break;
    case "mieru"_hash:
      ...
      mieruConstruct(node, MIERU_DEFAULT_GROUP, ps, port, password, server,
                     ports, user, multiplexing, transfer_protocol, udp,
                     tribool(), scv, tribool(), "");
      break;
    default:
      continue;
...
void explodeStdVMess(std::string vmess, Proxy &node) {
  ...
  vmessConstruct(node, V2RAY_DEFAULT_GROUP, remarks, add, port, type, id, aid,
                 net, "auto", path, host, "", tls, "", alpnList);
}

void explodeStdHysteria(std::string hysteria, Proxy &node) {
  std::string add, port, type, auth, host, insecure, up, down, alpn, obfsParam,
      remarks, auth_str, sni;
  std::string addition;
  ...
  hysteriaConstruct(node, HYSTERIA_DEFAULT_GROUP, remarks, add, port, type,
                    auth, auth_str, host, up, down, alpn, obfsParam, insecure,
                    "", sni);
  return;
}

void explodeStdMieru(std::string mieru, Proxy &node) {
  std::string username, password, host, port, ports, profile, protocol,
      multiplexing, mtu, remarks;
  std::string addition;
  ...
  // 账号密码@host
  if (regGetMatch(mieru, R"(^(.*?):(.*?)@(.*)$)", 4, 0, &username, &password,
                  &host))
    return;
  ...
  mieruConstruct(node, "MieruGroup", remarks, port, password, host, ports,
                 username, multiplexing, protocol, udp, tfo, scv, tls13, "");
}

void explodeStdHysteria2(std::string hysteria2, Proxy &node) {
  std::string add, port, password, host, insecure, up, down, alpn, obfsParam,
      obfsPassword, remarks, sni, ports;
  std::string addition;
  tribool scv;
  ...
  if (strFind(hysteria2, "@")) {
    if (regGetMatch(hysteria2, R"(^(.*?)@(.*)[:](\d+)$)", 4, 0, &password, &add,
                    &port))
      return;
  } else {
  ...
  hysteria2Construct(node, HYSTERIA2_DEFAULT_GROUP, remarks, add, port,
                     password, host, up, down, alpn, obfsParam, obfsPassword,
                     host, "", ports, tribool(), tribool(), scv);
  return;
}

void explodeStdVless(std::string vless, Proxy &node) {
  std::string add, port, type, id, aid, net, flow, pbk, sid, fp, mode, path,
      host, tls, remarks, sni;
  std::string addition;
  ...
  case "quic"_hash:
    type = getUrlArg(addition, "headerType");
    host =
        getUrlArg(addition, strFind(addition, "sni") ? "sni" : "quicSecurity");
    path = getUrlArg(addition, "key");
    break;
  default:
    return;
  ...
  if (remarks.empty())
    remarks = add + ":" + port;
  sni = getUrlArg(addition, "sni");
  vlessConstruct(node, XRAY_DEFAULT_GROUP, remarks, add, port, type, id, aid,
                 net, "auto", flow, mode, path, host, "", tls, pbk, sid, fp,
                 sni, alpnList, packet_encoding);
  return;
}
...
void explodeShadowrocket(std::string rocket, Proxy &node) {
  ...
  if (regGetMatch(urlSafeBase64Decode(rocket), "(.*?):(.*)@(.*):(.*)", 5, 0,
                  &cipher, &id, &add, &port))
    return;
  if (port == "0")
    return;
  ...
  vmessConstruct(node, V2RAY_DEFAULT_GROUP, remarks, add, port, type, id, aid,
                 net, cipher, path, host, "", tls, "", alpnList);
}
...
void parsePeers(Proxy &node, const std::string &data) {
  ...
  auto peer = peers[0];
  auto peerdata =
      regGetAllMatch(peer, R"(([a-z-]+) ?= ?([^" ),]+|".*?"),? ?)", true);
...
bool explodeSurge(std::string surge, std::vector<Proxy> &nodes) {
  ...
  */
  ...
  for (auto &x : proxies) {
    std::string remarks, server, port, method, username, password,
        sni; // common
    std::string plugin, pluginopts, pluginopts_mode, pluginopts_host, mod_url,
        mod_md5;                                    // ss
    std::string id, net, tls, host, edge, path, fp; // v2
    std::string protocol, protoparam;               // ssr
    std::string section, ip, ipv6, private_key, public_key, mtu, test_url,
        client_id, peer, keepalive; // wireguard
    ...
    */
    ...
    case "custom"_hash: // surge 2 style custom proxy
                        // remove module detection to speed up parsing and
                        // compatible with broken module
      /*
      ...
      */
      ...
      // if(mod_md5 == modSSMD5) //is SSEncrypt module
      {
        ...
        ssConstruct(node, SS_DEFAULT_GROUP, remarks, server, port, password,
                    method, plugin, pluginopts, udp, tfo, scv);
      }
      // else
      //     continue;
      break;
    ...
    case "socks5"_hash: // surge 3 style socks5 proxy
      ...
      socksConstruct(node, SOCKS_DEFAULT_GROUP, remarks, server, port, username,
    ...
    case "vmess"_hash: // surge 4 style vmess proxy
      ...
      vmessConstruct(node, V2RAY_DEFAULT_GROUP, remarks, server, port, "", id,
                     aead, net, method, path, host, edge, tls, "",
                     std::vector<std::string>{}, udp, tfo, scv, tls13);
      break;
    case "http"_hash: // http proxy
      ...
      httpConstruct(node, HTTP_DEFAULT_GROUP, remarks, server, port, username,
    ...
    case "trojan"_hash: // surge 4 style trojan proxy
      ...
      trojanConstruct(node, TROJAN_DEFAULT_GROUP, remarks, server, port,
                      password, "", host, "", fp, sni,
                      std::vector<std::string>{}, true, udp, tfo, scv);
      break;
    case "snell"_hash:
      server = trim(configs[1]);
      port = trim(configs[2]);
      ...
      snellConstruct(node, SNELL_DEFAULT_GROUP, remarks, server, port, password,
                     plugin, host, to_int(version, 0), udp, tfo, scv);
      break;
    case "wireguard"_hash:
      ...
      wireguardConstruct(node, WG_DEFAULT_GROUP, remarks, "", "0", ip, ipv6,
                         private_key, "", "", dns_servers, mtu, keepalive,
                         test_url, "", udp, "");
    ...
    default:
      ...
      case "shadowsocks"_hash: // quantumult x style ss/ssr link
        ...
        if (!protocol.empty()) {
          ssrConstruct(node, SSR_DEFAULT_GROUP, remarks, server, port, protocol,
                       method, pluginopts_mode, password, pluginopts_host,
                       protoparam, udp, tfo, scv);
        } else {
          ssConstruct(node, SS_DEFAULT_GROUP, remarks, server, port, password,
                      method, plugin, pluginopts, udp, tfo, scv, tls13);
        }
        break;
      ...
      case "vless"_hash: // quantumult x style vless link
        ...
        if (remarks.empty())
          remarks = server + ":" + port;
        vlessConstruct(node, XRAY_DEFAULT_GROUP, remarks, server, port, "", id,
                       aead, net, method, "chrome", "", path, host, "", tls, "",
                       "", fp, sni, std::vector<std::string>{}, "", udp, tfo,
                       scv, tls13);
        break;
      case "trojan"_hash: // quantumult x style trojan link
        ...
        trojanConstruct(node, TROJAN_DEFAULT_GROUP, remarks, server, port,
                        password, "", host, "", fp, sni,
                        std::vector<std::string>{}, tls == "true", udp, tfo,
                        scv, tls13);
        break;
      case "http"_hash: // quantumult x style http links
        ...
        httpConstruct(node, HTTP_DEFAULT_GROUP, remarks, server, port, username,
                      password, tls == "true", tfo, scv, tls13);
        break;
      default:
        continue;
      }
  ...
  for (uint32_t i = 0; i < json["configs"].Size(); i++) {
    ...
    case 6: // ss/ssr
      ...
      } else // is ssr cipher
      {
        ...
        ssrConstruct(node, group, remarks, server, port, protocol, cipher, obfs,
...
void explodeNetchConf(std::string netch, std::vector<Proxy> &nodes) {
  ...
  for (uint32_t i = 0; i < json["Server"].Size(); i++) {
    Proxy node;
    explodeNetch(
        "Netch://" + base64Encode(json["Server"][i] | SerializeObject()), node);
...
void explodeSingboxTransport(rapidjson::Value &singboxNode, std::string &net,
                             std::string &host, std::string &path,
                             std::string edge) {
  if (singboxNode.HasMember("transport") &&
      singboxNode["transport"].IsObject()) {
...
void explodeSingbox(rapidjson::Value &outbounds, std::vector<Proxy> &nodes) {
  ...
  for (rapidjson::SizeType i = 0; i < outbounds.Size(); ++i) {
    if (outbounds[i].IsObject()) {
      std::string proxytype, ps, server, port, cipher, group, password, ports,
          tempPassword; // common
      std::string type = "none", id, aid = "0", net = "tcp", path, host, edge,
                  tls, sni;                                 // vmess
      std::string fp = "chrome", pbk, sid, packet_encoding; // vless
      ...
      if (singboxNode.HasMember("type") && singboxNode["type"].IsString()) {
        ...
        if (singboxNode.HasMember("tls") && singboxNode["tls"].IsObject()) {
          ...
          if (tlsObj.HasMember("enabled") && tlsObj["enabled"].IsBool() &&
              tlsObj["enabled"].GetBool()) {
            tls = "tls";
          }
          sni = GetMember(tlsObj, "server_name");
          if (tlsObj.HasMember("alpn") && tlsObj["alpn"].IsArray() &&
              !tlsObj["alpn"].Empty()) {
          ...
          if (tlsObj.HasMember("disable_sni") &&
              tlsObj["disable_sni"].IsBool()) {
            disableSni = tlsObj["disable_sni"].GetBool();
          }
          if (tlsObj.HasMember("certificate") &&
              tlsObj["certificate"].IsString()) {
          ...
          if (tlsObj.HasMember("reality") && tlsObj["reality"].IsObject()) {
            ...
            if (reality.HasMember("server_name") &&
                reality["server_name"].IsString()) {
              host = reality["server_name"].GetString();
            }
            if (reality.HasMember("public_key") &&
                reality["public_key"].IsString()) {
              pbk = reality["public_key"].GetString();
            }
            if (reality.HasMember("short_id") &&
                reality["short_id"].IsString()) {
          ...
          if (tlsObj.HasMember("utls") && tlsObj["utls"].IsObject()) {
            if (rapidjson::Value reality = tlsObj["utls"].GetObject();
                reality.HasMember("fingerprint") &&
                reality["fingerprint"].IsString()) {
        ...
        case "vmess"_hash:
          ...
          vmessConstruct(node, group, ps, server, port, "", id, aid, net,
                         cipher, path, host, edge, tls, sni, alpnList, udp, tfo,
                         scv);
          break;
        ...
        case "trojan"_hash:
          ...
          password = GetMember(singboxNode, "password");
          explodeSingboxTransport(singboxNode, net, host, path, edge);
          trojanConstruct(node, group, ps, server, port, password, net, host,
                          path, fp, sni, alpnList, true, udp, tfo, scv);
          break;
        case "vless"_hash:
          group = XRAY_DEFAULT_GROUP;
          id = GetMember(singboxNode, "uuid");
          ...
          if (singboxNode.HasMember("transport") &&
            ...
            case "ws"_hash: {
              path = GetMember(transport, "path");
              if (transport.HasMember("headers") &&
                  transport["headers"].IsObject()) {
                rapidjson::Value headers = transport["headers"].GetObject();
                host = GetMember(headers, "Host");
                edge = GetMember(headers, "Edge");
              }
          ...
          vlessConstruct(node, group, ps, server, port, type, id, aid, net,
                         "auto", flow, mode, path, host, "", tls, pbk, sid, fp,
                         sni, alpnList, packet_encoding, udp);
          break;
        case "http"_hash:
          password = GetMember(singboxNode, "password");
        ...
        case "wireguard"_hash:
          ...
          dns_server = {"8.8.8.8"};
          wireguardConstruct(node, group, ps, server, port, ip, ipv6,
                             private_key, public_key, password, dns_server, mtu,
                             "0", "", "", udp, "");
          break;
        case "socks"_hash:
          group = SOCKS_DEFAULT_GROUP;
        ...
        case "hysteria"_hash:
          ...
          hysteriaConstruct(node, group, ps, server, port, type, auth, "", host,
                            up, down, alpn, obfsParam, insecure, ports, sni,
                            udp, tfo, scv);
          break;
        case "anytls"_hash:
          group = ANYTLS_DEFAULT_GROUP;
          password = GetMember(singboxNode, "password");
          anyTlSConstruct(node, ANYTLS_DEFAULT_GROUP, ps, port, password,
                          server, alpnList, fingerprint, sni, udp, tribool(),
                          scv, tribool(), "", 30, 30, 0);
          break;
        case "hysteria2"_hash:
          group = HYSTERIA2_DEFAULT_GROUP;
          ...
          hysteria2Construct(node, group, ps, server, port, password, host, up,
                             down, alpn, obfsParam, obfsPassword, sni,
                             public_key, "", udp, tfo, scv);
          break;
        case "tuic"_hash:
          group = TUIC_DEFAULT_GROUP;
          password = GetMember(singboxNode, "password");
          ...
          if (singboxNode.HasMember("zero_rtt_handshake") &&
              singboxNode["zero_rtt_handshake"].IsBool()) {
            rrt = singboxNode["zero_rtt_handshake"].GetBool();
          }
          udp_relay_mode = GetMember(singboxNode, "udp_relay_mode");
          tuicConstruct(node, TUIC_DEFAULT_GROUP, ps, server, port, password,
                        congestion_control, alpn, sni, id, udp_relay_mode, "",
                        tribool(), tribool(), scv, rrt, disableSni);
          break;
        default:
          continue;
        }
...
void explodeTuic(const std::string &tuic, Proxy &node) {
  std::string add, port, password, host, insecure, alpn, remarks, sni, ports,
      congestion_control;
  std::string addition;
  tribool scv;
  ...
  if (remarks.empty())
    remarks = add + ":" + port;
  tuicConstruct(node, TUIC_DEFAULT_GROUP, remarks, add, port, password,
                congestion_control, alpn, sni, uuid, "native", "", tribool(),
                tribool(), scv);
...
void explodeAnyTLS(std::string anytls, Proxy &node) {
  ...
  anyTlSConstruct(node, ANYTLS_DEFAULT_GROUP, remarks, port, password, add,
                  alpnList, fp, sni, udp, tfo, scv, tribool(), "", 30, 30, 0);
}
...
void explode(const std::string &link, Proxy &node) {
  if (startsWith(link, "ssr://"))
    ...
    explodeSocks(link, node);
           ...
           startsWith(link, "tg://http")) // telegram style http link
...
void explodeSubLink(std::string link, std::vector<Proxy> &nodes) {
  ...
  if (addNodes(link, nodes, 0, settings) == -1) {
    writeLog(0,
             "The following link doesn't contain any valid node info: " + link,
             LOG_LEVEL_WARNING);
  }
}
